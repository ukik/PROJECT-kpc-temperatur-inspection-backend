...
if (!function_exists('JWTCreate')) {
    function JWTCreate($payload)
    {
        ...
        $custom_claims = [
            'exp'       => $expiration_date,
            'scopes'    => [TrimString(ucfirst($payload->position_employee))],
...
if (!function_exists('JWTToken')) {
    function JWTToken()
    {
        try {
            ...
            return fallback(['token_expired'], $e->getStatusCode());
        } catch (TokenInvalidException $e) {
            return fallback(['token_invalid'], $e->getStatusCode());
        } catch (JWTException $e) {
            return fallback(['token_absent'], $e->getStatusCode());
        }
    }
}
    ...
    function JWTRevoke()
    {
        try {
            ...
            if (!JWTAuth::parseToken()->invalidate()) {
                return fallback(['user_not_found'], 404);
            }
        } catch (TokenExpiredException $e) {
            return fallback(['token_expired'], $e->getStatusCode());
        } catch (TokenInvalidException $e) {
