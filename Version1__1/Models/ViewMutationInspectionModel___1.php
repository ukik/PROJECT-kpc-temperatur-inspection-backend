...
class ViewMutationInspectionModel___1 extends Model
{
  ...
  public function __construct($attributes = [])
  {
    ...
    // dd(SegmentExist('inspection-information'));
    ...
    if (!$year && !$month) {
      ...
      if ($year <= date('Y') && $month <= date('m')) {
        ...
        if (!ifTableExist("tb_mutation_inspection_{$year}_{$month}")) {
          $this->createStaticTableMutationInspectionSchema("tb_mutation_inspection_{$year}_{$month}");
        }
        if (!ifTableExist("tb_mutation_inspection_information_{$year}_{$month}")) {
          $this->createStaticTableMutationInspectionInformationSchema("tb_mutation_inspection_information_{$year}_{$month}", "_{$year}_{$month}");
        }

        // mirror
        if (!ifViewExist("vw_mutation_inspection_information_{$year}_{$month}")) {
          $this->createStaticViewMutationInspectionSchema("vw_mutation_inspection_information_{$year}_{$month}");
        }
        if (!ifViewExist("vw_mutation_inspection_information_{$year}_{$month}")) {
          $this->createStaticViewMutationInspectionInformationSchema("vw_mutation_inspection_information_{$year}_{$month}", "_{$year}_{$month}");
        }
      }

      $this->table = SegmentExist('inspection-information') ? "vw_mutation_inspection_information_{$year}_{$month}" : "vw_mutation_inspection_{$year}_{$month}";
      $this->table_session = SegmentExist('inspection-information') ? "vw_mutation_inspection_information_{$year}_{$month}" : "vw_mutation_inspection_{$year}_{$month}";
